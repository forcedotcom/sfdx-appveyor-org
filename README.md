...
This repository shows how to successfully setup deploying to non-scratch orgs (i.e. Sandbox or Production) with Appveyor. We make a few assumptions in this README:

- You know how to get your repository setup with Appveyor. (Here's their [Getting Started guide](https://www.appveyor.com/docs/enterprise/).)
...
7) From your JWT-Based connected app on Salesforce, retrieve the generated `Consumer Key` and store in a AppVeyor environment variable named `CONSUMER_KEY` using the AppVeyor UI.

8) Store the user name that you use to access your target org in a AppVeyor environment variable named `USER_NAME` using the AppVeyor UI. Note that this username is the username that you use to login into your target org.
